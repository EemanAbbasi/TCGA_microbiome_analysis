...
def run_global(models,comm_id, community, organisms, medium_id, excluded_mets, env, verbose, min_mol_weight, use_lp, debug=True):
    global_data = []
    debug_data = []
    ...
    global_data.append((models, comm_id, medium_id, len(organisms), mip,mip_extras, mro))

    return global_data, debug_data


def run_detailed(models, comm_id, community, medium_id, excluded_mets, env, verbose, min_mol_weight, ignore_coupling):
    smt_data = []
    ...
    for org1, org2 in pairs:
        ...
        for met in sorted(metabolites):
            ...
            smt_data.append((models,comm_id, medium_id, org1, org2, met, scs_o1_o2, mus_o1_met, mps_o2_met, smt))
...
def export_results(mode, models, output, data, debug_data, zeros):
    prefix = output + '_' if output else ''

    name = str(models)
    names = (name.replace("[('all',", ''))
    names = str(names).replace('[','').replace(']','').replace('\'','').replace('\"','').replace(')','')

    if mode == "global":

        outputFileName = names + "_global.tsv"
        outputDebugName = names + "_debug.tsv"


        df = pd.DataFrame(data, columns=['community', 'medium', 'size', 'models',"mip", "mip_extras", "mro"])
        df.to_csv(prefix + outputFileName, sep='\t', index=False)

        if len(debug_data) > 0:
            df = pd.DataFrame(debug_data, columns=['community', 'medium', 'key1', "key2", "data"])
            df.to_csv(prefix + outputDebugName, sep='\t', index=False)

    else:
        outputFileName = names + "_detailed.tsv"
        columns = ['community', 'models','medium','receiver', 'donor', 'compound', 'scs', 'mus', 'mps', 'smetana']
...
def main(models, communities=None, mode=None, output=None, flavor=None, media=None, mediadb=None, aerobic=None,
         zeros=False,verbose=False, min_mol_weight=False, use_lp=False, exclude=None, debug=True ,
    ...
    for comm_id, organisms in list(comm_dict.items()):
        ...
        for medium in media:
            ...
            if mode == "global":
                entries, debug_entries = run_global(models,comm_id, community, organisms, medium_id, excluded_mets, env,
                                                    verbose, min_mol_weight, use_lp, debug)
                debug_data.extend(debug_entries)

            if mode == "detailed":
                entries = run_detailed(models,comm_id, community, medium_id, excluded_mets, env, verbose, min_mol_weight,
                                       ignore_coupling)
            ...
            #if debug and mode=="global":
                #debug_data.extend(debug_entries)

    export_results(mode,list(comm_dict.items()), output, data, debug_data, zeros)
